...
- **Land the decision, don't just record it.** An issue comment isn't the source of truth — a decision is done when it's in `rules/` *and* the engine. The two directions aren't symmetric: rules describing something unbuilt is a gap (file the engine issue), but engine behaviour the rules don't describe is the engine inventing rules, and reads as a bug to the next reviewer. If only one can land now, land `rules/` and file the gap — never narrow the rules to match what got built, and never park an untracked gap in a `[design:]` note.
